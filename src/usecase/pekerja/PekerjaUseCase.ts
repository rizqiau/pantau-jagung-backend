...
export class PekerjaUseCase {
  ...
  static async updateUser(id: string, payload: any) {
    ...
    const dataToUpdate: any = { nama, username, role };
  ...
  static async deleteUser(id: string, operatorId?: string | null) {
    return await prisma.$transaction(async (tx) => {
  ...
  static async addPekerjaHarian(payload: any) {
  ...
  static async updatePekerjaHarian(id: string, payload: any) {
    const { nama, peran, operatorId } = payload;
    return await prisma.$transaction(async (tx) => {
  ...
  static async deletePekerjaHarian(id: string, operatorId?: string | null) {
    return await prisma.$transaction(async (tx) => {
      const pekerja = await tx.pekerjaHarian.findUnique({ where: { id } });
      if (!pekerja) throw new Error("Pekerja harian tidak ditemukan");
