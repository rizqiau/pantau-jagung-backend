...
export class VerifikasiUseCase {
    ...
    static async createVerifikasi(payload) {
        const { id, blokId, jadwalId, mandorId, tanggal, catatan, listHasilKerja, hasilKerjaPekerja } = payload;
    ...
    static async getVerifikasiByBlok(blokId) {
    ...
    static async batasiAtauVerifikasiLaporan(id, asistenId, status) {
        ...
        if (!laporan)
            throw new Error("Laporan verifikasi tidak ditemukan");
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (status === "APPROVED") {
                const totalLuas = laporan.hasilKerjaPekerja.reduce((sum, item) => sum + item.luasDikerjakan, 0);
                if (totalLuas > 0) {
...
//# sourceMappingURL=VerifikasiUseCase.js.map