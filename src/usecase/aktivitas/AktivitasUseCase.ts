...
export class AktivitasUseCase {
  static async inputAktivitasHarian(payload: any) {
    const {
    ...
    if (!jadwal) throw new Error("Jadwal pekerjaan tidak ditemukan");

    let totalBiayaPekerja = 0;
    if (listPekerja && listPekerja.length > 0) {
      totalBiayaPekerja = listPekerja.reduce(
        (sum: number, p: any) => sum + p.upahHarian,
        0,
      );
    }

    let totalBiayaMaterial = 0;
    const pemakaianMaterialData: any[] = [];
    ...
    if (listMaterial && listMaterial.length > 0) {
      for (const item of listMaterial) {
        ...
        if (materialMaster.stok < item.jumlahPakai) {
          throw new Error(
            `Stok ${materialMaster.namaMaterial} tidak mencukupi. Sisa stok: ${materialMaster.stok}`,
          );
        }
  ...
  static async getAllAktivitas() {
    ...
    return listAktivitas.map((aktivitas) => ({
      ...
      totalBiayaKeseluruhan:
        aktivitas.totalBiayaPekerja + aktivitas.totalBiayaMaterial,
    }));
  }
}
