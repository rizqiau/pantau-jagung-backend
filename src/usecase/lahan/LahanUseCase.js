...
export class LahanUseCase {
    ...
    static async updateLahan(id, payload) {
    ...
    static async deleteLahan(id, operatorId) {
        return await prisma.$transaction(async (tx) => {
    ...
    static async rescheduleJadwal(jadwalId, tanggalBaruString, operatorId) {
        ...
        const selisihHari = differenceInDays(tanggalBaru, targetJadwal.tanggalJadwal);
        if (selisihHari === 0) {
            return {
        ...
        for (const jadwal of jadwalTerdampak) {
            ...
            transactionKueri.push(prisma.jadwalPerawatan.update({
                where: { id: jadwal.id },
                data: { tanggalJadwal: tanggalDiperbarui },
            }));
        }
        // E. Update estimasi panen di Siklus Tanam
        const estimasiPanenBaru = addDays(targetJadwal.siklus.estimasiPanen, selisihHari);
        transactionKueri.push(prisma.siklusTanam.update({
            where: { id: targetJadwal.siklusId },
            data: { estimasiPanen: estimasiPanenBaru },
        }));
        // F. Catat Audit Log
        transactionKueri.push(prisma.auditLog.create({
            data: {
                userId: operatorId || null,
                action: "UPDATE",
...
//# sourceMappingURL=LahanUseCase.js.map