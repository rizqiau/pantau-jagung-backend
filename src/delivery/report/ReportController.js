...
export class ReportController {
    static async getCostReport(req, res) {
        try {
            const lahanId = req.query.lahanId;
    ...
    static async getHarvestReport(req, res) {
        try {
            const lahanId = req.query.lahanId;
    ...
    static async downloadPdf(req, res) {
        try {
            ...
            costData.perKegiatan.forEach((item, idx) => {
            ...
            doc.moveDown(0.5);
            doc.fontSize(10).font("Helvetica");
            costData.perLahan.forEach((item, idx) => {
                doc.text(`${idx + 1}. ${item.nama}: ${formatRp(item.total)}`);
            });
            doc.moveDown(1);
            // ─── DETAIL KEGIATAN ───
            if (costData.detailKegiatan.length > 0) {
                ...
                doc.moveDown(0.5);
                costData.detailKegiatan.forEach((dk) => {
                    ...
                    if (dk.rincianHK.length > 0) {
                        doc.text(`  Rincian HK: ${dk.rincianHK.map((h) => `${h.nama} (${formatRp(h.upah)})`).join(", ")}`);
                    }
                    if (dk.rincianMaterial.length > 0) {
                        doc.text(`  Rincian Material: ${dk.rincianMaterial.map((m) => `${m.nama} ${m.jumlah} ${m.satuan} (${formatRp(m.total)})`).join(", ")}`);
                    }
                    doc.moveDown(0.5);
                });
            }
            ...
            doc.moveDown(0.5);
            doc.fontSize(10).font("Helvetica");
            harvestData.perLahan.forEach((item, idx) => {
