// File: src/delivery/PanenController.ts
...
export class PanenController {
  ...
  static async getBySiklus(req: Request, res: Response) {
    try {
      const siklusId = req.params.siklusId as string;
      if (!siklusId) throw new Error("ID Siklus wajib disertakan");
  ...
  static async updateStatus(req: Request, res: Response) {
    try {
      ...
      if (!id) throw new Error("ID Laporan Panen wajib disertakan");
      if (!asistenId || !status) {
      ...
      const hasilVerifikasi = await PanenUseCase.verifikasiPanen(
        id,
        asistenId,
        status,
      );
