...
export class BlokUseCase {
  ...
  static async getBlokProgressByLahan(lahanId: string) {
    ...
    return bloks.map((blok) => {
      ...
      const kegiatanSaatIni =
        siklusAktif.jadwalPerawatan.find((j) => j.status === "TERJADWAL") ||
  ...
  static async getRiwayatPekerjaanByJadwal(jadwalId: string) {
