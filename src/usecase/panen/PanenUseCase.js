...
export class PanenUseCase {
    ...
    static async inputPanen(payload) {
        const { id, // UUID opsional dari Android
        siklusId, mandorId, tanggalPanen, beratHasil, hargaJualPerKg, kualitas, catatan, } = payload;
        if (id) {
        ...
        if (!siklus)
            throw new Error("Siklus tanam tidak ditemukan");
        // B. Validasi Mandor
        const mandor = await prisma.user.findUnique({
            where: { id: mandorId },
        });
        if (!mandor)
            throw new Error("Mandor tidak ditemukan");
    ...
    static async getPanenBySiklus(siklusId) {
        return await prisma.panen.findMany({
            where: { siklusId },
            orderBy: { tanggalPanen: "desc" },
            include: {
    ...
    static async verifikasiPanen(id, asistenId, status) {
        ...
        if (!panen)
            throw new Error("Laporan panen tidak ditemukan");
        // B. Validasi Pemeriksa (Asisten Lapangan)
        const asisten = await prisma.user.findUnique({
            where: { id: asistenId },
        });
        if (!asisten)
            throw new Error("Asisten Lapangan tidak ditemukan");
...
//# sourceMappingURL=PanenUseCase.js.map