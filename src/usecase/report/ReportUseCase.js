...
export class ReportUseCase {
     ...
     */
    static async getCostReport(lahanId) {
        ...
        const perMinggu = {};
        const perKegiatan = {};
        const biayaKumulatifMap = {};
        let totalBiayaSeluruhnya = 0;
        let totalBiayaHK = 0;
        let totalBiayaMaterial = 0;
        const uniquePekerja = new Set();
        ...
        aktivitas.forEach((a) => {
            ...
            // Aggregate per Lahan
            if (!perLahan[lahanIdCur])
                perLahan[lahanIdCur] = { nama: namaLahan, total: 0 };
            perLahan[lahanIdCur].total += biaya;
            // Aggregate per Blok
            if (!perBlok[blokId])
                perBlok[blokId] = { nama: namaBlok, total: 0 };
            perBlok[blokId].total += biaya;
            // Aggregate per Siklus
            if (!perSiklus[siklusId])
                perSiklus[siklusId] = { nama: namaSiklus, total: 0 };
            perSiklus[siklusId].total += biaya;
            // Aggregate per Jadwal
            if (!perJadwal[jadwalId])
                perJadwal[jadwalId] = { nama: namaJadwal, total: 0 };
            perJadwal[jadwalId].total += biaya;
            // Aggregate per Hari
            if (!perHari[hariStr])
                perHari[hariStr] = { nama: hariStr, total: 0 };
            perHari[hariStr].total += biaya;
            // Aggregate per Minggu
            if (!perMinggu[mingguStr])
                perMinggu[mingguStr] = { nama: mingguStr, total: 0 };
            perMinggu[mingguStr].total += biaya;
            // Aggregate per Kegiatan (untuk donut chart)
            if (!perKegiatan[namaKegiatan])
                perKegiatan[namaKegiatan] = { nama: namaKegiatan, total: 0 };
        ...
        const formatArr = (obj) => Object.values(obj).sort((a, b) => b.total - a.total);
        const formatDateArr = (obj) => Object.values(obj).sort((a, b) => a.nama.localeCompare(b.nama));
        return {
     ...
     */
    static async getHarvestReport(lahanId) {
        ...
        panen.forEach((p) => {
            ...
            if (!perLahan[lahanIdCur])
                perLahan[lahanIdCur] = { nama: namaLahan, totalBerat: 0, totalPendapatan: 0 };
            perLahan[lahanIdCur].totalBerat += p.beratHasil;
            perLahan[lahanIdCur].totalPendapatan += p.totalPendapatan;
            if (!perBulan[bulanStr])
                perBulan[bulanStr] = { nama: bulanStr, totalBerat: 0, totalPendapatan: 0 };
            perBulan[bulanStr].totalBerat += p.beratHasil;
            perBulan[bulanStr].totalPendapatan += p.totalPendapatan;
        });
        const formatArray = (obj) => Object.values(obj);
        return {
            totalBeratSeluruhnya,
            totalPendapatanSeluruhnya,
            perLahan: formatArray(perLahan).sort((a, b) => b.totalBerat - a.totalBerat),
            perBulan: formatArray(perBulan).sort((a, b) => a.nama.localeCompare(b.nama)),
        };
    }
}
//# sourceMappingURL=ReportUseCase.js.map